...
- `jogos/dialog/CanaisDialogFragment.java` — modal (bottom sheet) com os **canais da playlist (`canais_links`)** do jogo (única seção exibida — `Canais:`); aberto ao clicar na linha do jogo (ver `docs/CANAIS_TRANSMISSAO.md`)
...
- `jogos/event/EsporteEventListener.java` — **eventos para o app consumidor**: holder estático com dois listeners — `EsporteEventCallback` (`onJogoClicado(ItemJogos)`, disparado no clique da linha do jogo em `ActivityEsporte.setList()` junto com a abertura do modal; registro via `setListener(...)`) e `AoClicarNoCanalListener` (`boolean aoClicarNoCanal(int idCanal)`, disparado no clique de um canal da seção "Canais:" do modal `CanaisDialogFragment`; registro via `onChannelClickListener(...)`; entrega o `stream_id` do canal). Limpeza com `clear()` limpa os dois (ver `docs/EVENT_LISTENER.md`)
...
- `jogos/item/ItemClassificacao.java` + `jogos/response/ApiClassificacaoCaller.java` + `jogos/bancoSql/ClassificacaoDatabase.java` — tabela de classificação (`GET /api/campeonato/{id}/classificacao`); opção "Classificação" aparece acima de "HOJE" ao selecionar um campeonato (ver `docs/CLASSIFICACAO.md`)
  ...
  - `res/layout/` — `frame_esportes.xml` (tela principal), `api_category`, `api_item_jogos`, `api_item_canal_linha` (chip da faixa), `api_item_data`, `api_item_camp`, `api_expired`, classificação: `api_classificacao_header`, `api_item_classificacao`, `api_item_classificacao_promocao` (barra de zona, não focável), `api_item_classificacao_opcao` (padrão das datas)
...
- **Publicada a tag `1.8`** (commit `324ba31`, branch `feature/layout-widget`):
  - **Navegação da classificação travada corrigida (Android TV) — causa raiz:** a API retorna 20 times e o banco Room (`classificacao.db`, camp 1957) guarda 20 (dado íntegro, o adapter renderiza os 20). O bug real era **foco D-pad preso** ao cruzar as **barras de zona de promoção** (`api_item_classificacao_promocao.xml`, ex.: "LIBERTADORES"/"SUDAMERICANA") e o cabeçalho (`api_classificacao_header.xml`), que **não são focáveis** — o `requestFocus` caía na barra e a navegação parava no fim do bloco (não passava de Libertadores para Série B). Swipe (toque) rolava normalmente (confirmado na TV).
  - **Fix:** `ActivityEsporte.navegarListaJogos()` agora **pula posições não focáveis** quando o adapter é `ClassificacaoAdapter` — `while (alvo >= 0 && alvo < count && !classificacao.isFocavel(alvo)) alvo += delta;` — usando o novo `ClassificacaoAdapter.isFocavel(int)` (posição 0 = header → `false`; `PromocaoItem` → `false`; demais → `true`). O caminho `scrollToPosition` + `focarItemAposLayout()` (requestLayout) **funciona** — o travamento anterior era só a barra não focável. Verificado na TV (Betano, camp 1957): log real `pos=5 alvo=7 count=25 childAlvo=false canScrollDown=true` (pulou a barra da posição 6); percorre até a posição 20 (`canScrollDown=false`) e volta (UP) sem travar. A lista de jogos **não é afetada** (skip só para `ClassificacaoAdapter`). Logs de instrumentação removidos após validação.
  - **Opção "Tabela" → "Classificação":** string `opcao_classificacao` = "Classificação"; `api_item_classificacao_opcao.xml` reescrito no **padrão de `api_item_data.xml`** (TextView `tvData`, fundo `bg_data_item`, altura `_36sdp`, fonte `_6sdp`, cor `widget_text`, bold, focusable); `DataAdapter.bindClassificacao` usa `holder.tvData.setSelected(...)` e clique no `tvData`. `ic_trophy_white`/`bg_classificacao_item` ficaram **sem uso** (mantidos no repo).
  - **Modal de canais:** `strings.xml` `modal_canais_sec_links` = **"Canais:"** (era "Links"); `modal_canais_title` = "Canais de Transmissão".
  - Testes: build `:app:assembleDebug`/`:demo:assembleDebug` OK; APK instalado na TV `192.168.3.14:5555`; validação da opção "Classificação" (aparece acima de HOJE, selected funciona, abre a classificação) e da navegação por blocos.
  - **Não commitado:** `exemple/` e `res/` (raiz) continuam untracked.
- **Publicada a tag `1.7`** (commit `6d9c004`, branch `feature/layout-widget`): redesign de layout/UX — pills de status do jogo na identidade do widget web (`JogoStatus.pillRes()` → `bg_pill_*`), cards de jogos (`bg_jogo_card*`), seletor de campeonato, item de data, faixa de canais e `frame_esportes`; modal de canais passou a exibir **apenas** `canais_links` (playlist). Docs parcialmente atualizados nessa tag (README ficou com referências a `1.5`/"Tabela" até esta atualização).
...
- **Implementado o Event Listener** (solicitação de cliente que consome o SDK via importação; decisões: clique na **linha do jogo** entrega **todos** os `canais_links`; manter o modal interno; entrega via **callback estático**):
  ...
  - **Publicado dentro da tag `1.2`** — **contexto atual:** a `1.2` ficou com o build ANTIGO no cache do JitPack (o cliente não via os métodos novos). Por isso foi **publicada a tag `1.3`** no mesmo commit (`fa4033b`, branch `feature/classificacao-canais` mantida) — o cliente deve usar `com.github.jaderesp:api_esportes:1.3`. A `1.2` continua existindo, mas com build antigo (não usar).
- **Adicionado o Event Listener de canal (solicitação do programador do cliente):** dispara no **clique de um canal na seção "Canais:" dentro do modal** `CanaisDialogFragment`; entrega **somente o `stream_id` (int)**:
...
- **Novas necessidades do cliente implementadas (pedido do programador do cliente):**
  ...
  2. **Feedback de foco nos chips do modal (TV):** novo drawable `bg_canal_chip_selector_modal.xml` (normal = `bg_canal_chip`, focado/selecionado/pressionado = `bg_canal_chip_foco`); aplicado nos chips do modal (seção `Canais:`).
  3. **Logotipo do canal nos chips:** o chip da seção de links do modal virou um `LinearLayout` horizontal com `ImageView` (logo) + `TextView` (nome); o logo só é exibido quando `channel_logo` não é vazio, carregado via `ImageLoader`.
...
- **Implementados os canais de transmissão** (modal + faixa na lista):
  ...
  - `CanaisDialogFragment`: exibe **apenas** a seção `Canais:` (`canais_links`, objetos via `ARG_CANAIS_LINKS_JSON`, Gson `TypeToken<List<ItemCanalLink>>`); os índices `canais`, `canais_ia` e `canais_simples` **não** são listados (não têm `transmission_url` — não são clicáveis); vazio → "Nenhum canal informado".
